   ...
   if (type === VARIABLE_CONSTANTS.EVENT.ERROR && !skipDefaultNotification) {
    ...
    if (type === VARIABLE_CONSTANTS.EVENT.BEFORE_UPDATE) {
        if (variable.category === 'wm.LiveVariable' && variable.operation === 'read') {
            return $parseEvent(variable[type],  variable._context, {variable: variable, dataFilter: data, options: options});
        } else {
    
            return $parseEvent(variable[type], variable._context, {variable: variable, inputData: data, options: options});
        }
    } else {
        return $parseEvent(variable[type], variable._context, {variable: variable, data: data, options: options});
    }
};
