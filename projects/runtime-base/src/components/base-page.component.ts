...
import {each, extend, get} from "lodash-es";
...
export abstract class BasePageComponent extends FragmentMonitor implements AfterViewInit, OnDestroy, AfterContentInit {
    ...
    executeCallback(callback: string, event: Event) {
        ...
        if (functionMatch) {
            ...
            let target = event.currentTarget && (event.currentTarget as any).widget;
            ...
            if (target && this[functionName]) {
                ...
                if (callbackPrefix) {
                    if (callbackPrefix.endsWith('.')) {
                        ...
                        params.length ? x[functionName].call(params) : x[functionName]();
