    ...
    onValueChange(val) {
        if (!this.isDestroyed) {
            ...
            if (captionEl.length > 0) {
                ...
                this.app.notify('captionPositionAnimate', {isSelect: true, displayVal: hasValue, nativeEl: captionEl, isSelectMultiple: this.formWidget && this.formWidget.multiple, isFocused: this._activeField});
            }
           // Fix for [WMS-23959]: ADA issue - adding aria-describedby, aria-invalid, __errormsg attributes only when the form field is invalid , else removing the attributes
            this.setAriaAttributes();
