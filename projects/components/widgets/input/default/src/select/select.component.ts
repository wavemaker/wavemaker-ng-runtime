...
export class SelectComponent extends DatasetAwareFormComponent implements AfterViewInit {
    ...
    checkForFloatingLabel($event) {
        ...
        if (captionEl.length > 0) {
            if ((!$event || $event.type === 'focus') && (($(this.selectEl).find('select option:selected').text() === '' && (this.placeholder || this.datavalue)))) {
                ...
                if(typeof(this.datavalue) !== 'object' &&  this.datavalue != "") {
                    $(this.selectEl.nativeElement).find('option:selected').text(this.datavalue);
                }
            }
