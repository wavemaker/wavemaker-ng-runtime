...
import { ALLFIELDS, convertDataToObject, DataSetItem, extractDataAsArray, getOrderedDataset, getUniqObjsByDataField, handleHeaderClick, toggleAllHeaders, transformFormData, transformDataWithKeys, groupData, ToDatePipe, WIDGET_CONFIG, IWidgetConfig } from '@wm/components/base';
...
export abstract class DatasetAwareFormComponent extends BaseFormCustomComponent implements OnInit {
    ...
    protected constructor(inj: Injector,  @Inject(WIDGET_CONFIG) config: IWidgetConfig, @Attribute('groupby') public groupby?: string) {
        super(inj, config);
