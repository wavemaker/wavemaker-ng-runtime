...
describe('ButtonGroupDirective', () => {
    ...
    it('should not select a button when a non-button element is clicked', () => {
        buttonGroupElement.nativeElement.click();
        fixture.detectChanges();
        const buttons = buttonGroupElement.queryAll(By.css('.app-button'));
        expect(buttons.every(button => !button.nativeElement.classList.contains('selected'))).toBeTruthy();
    });
});