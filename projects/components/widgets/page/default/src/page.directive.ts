...
export class PageDirective extends StylableComponent implements AfterViewInit, OnDestroy {
    ...
    public ngAfterViewInit() {
        ...
        this.screen._eventNotifier.subscribe('on-resize', data => {
            this.notify('resize', data);
            this.invokeEventCallback('resize', { data, widget: this });
        });
        this.screen._eventNotifier.subscribe('on-orientationchange', data => {
            this.notify('orientationchange', data);
