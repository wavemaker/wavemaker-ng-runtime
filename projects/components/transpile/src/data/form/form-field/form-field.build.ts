...
const registerFormField = (isFormField): IBuildTaskDef => {
    return {
        ...
        pre: (attrs, shared, parentForm, parentLiveForm, parentFilter, parentList) => {
            ...
            return `<${tagName}  [ngStyle]="{'height': '${widgetHeight}'}" data-role="${dataRole}" [formGroup]="${pCounter}.ngform" wmFormField #${counter}="wmFormField" widgettype="${widgetType}" ${getFormMarkupAttr(attrs)}>
                        <div [ngStyle]="{'height': '${widgetHeight}'}" class="live-field form-group app-composite-widget clearfix caption-{{${pCounter}.captionposition}}" widget="${widgetType}">
                            ...
                            <div [ngStyle]="{'height': '${widgetHeight}'}" [ngClass]="${counter}.displayname ? ${pCounter}._widgetClass : '${controlLayout}'">
